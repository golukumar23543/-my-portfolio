    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-secondary-dark border border-white/10 rounded-2xl w-full max-w-md p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form className="space-y-4" onSubmit={mode === 'login' ? handleLogin : mode === 'register' ? handleRegister : handleForgot}>
          ...
          <button disabled={isLoading} type="submit" className={`w-full bg-accent-blue hover:bg-accent-blue-hover text-primary-dark font-bold py-3.5 rounded-xl transition-all mt-4 ${isLoading ? 'opacity-70 cursor-wait' : ''}`}>
        ...
        {(mode === 'login' || mode === 'register') && (
          <>
            ...
            <button 
              ...
              disabled={isLoading}
              className={`w-full bg-white hover:bg-gray-100 text-black font-bold py-3.5 rounded-xl transition-all flex justify-center items-center gap-2 shadow-lg ${isLoading ? 'opacity-70 cursor-wait' : ''}`}
            >
